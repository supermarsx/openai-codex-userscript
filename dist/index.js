...
const suggestions_1 = require("./helpers/suggestions");
...
`;
    ...
    let suggestions = (0, suggestions_1.loadSuggestions)() || suggestions_1.DEFAULT_SUGGESTIONS.slice();
    let options = (0, options_1.loadOptions)();
    let history = (0, history_1.loadHistory)();
        ...
        suggestions.forEach((s, i) => {
            ...
            edit.addEventListener('click', () => {
                ...
                if (inp !== null) {
                    ...
                    (0, suggestions_1.saveSuggestions)(suggestions);
                    renderSuggestions();
                    refreshDropdown();
                }
            });
    ...
    historyModal.addEventListener('click', (e) => { if (e.target === historyModal)
        historyModal.classList.remove('show'); });
    historyModal.querySelector('#gpt-history-clear').addEventListener('click', () => { history = []; (0, history_1.saveHistory)(history); renderHistory(); });
    modal.querySelector('#gpt-setting-theme').addEventListener('change', (e) => { options.theme = e.target.value; (0, options_1.saveOptions)(options); applyOptions(); });
    modal.querySelector('#gpt-setting-header').addEventListener('change', (e) => { options.hideHeader = e.target.checked; (0, options_1.saveOptions)(options); applyOptions(); });
    modal.querySelector('#gpt-setting-docs').addEventListener('change', (e) => { options.hideDocs = e.target.checked; (0, options_1.saveOptions)(options); applyOptions(); });
    modal.querySelector('#gpt-setting-logo-text').addEventListener('change', (e) => { options.hideLogoText = e.target.checked; (0, options_1.saveOptions)(options); applyOptions(); });
    modal.querySelector('#gpt-setting-logo-image').addEventListener('change', (e) => { options.hideLogoImage = e.target.checked; (0, options_1.saveOptions)(options); applyOptions(); });
    modal.querySelector('#gpt-setting-profile').addEventListener('change', (e) => { options.hideProfile = e.target.checked; (0, options_1.saveOptions)(options); applyOptions(); });
    modal.querySelector('#gpt-setting-environments').addEventListener('change', (e) => { options.hideEnvironments = e.target.checked; (0, options_1.saveOptions)(options); applyOptions(); });
    modal.querySelector('#gpt-setting-auto-updates').addEventListener('change', (e) => { options.autoCheckUpdates = e.target.checked; (0, options_1.saveOptions)(options); });
    ...
    function findSendButton() {
        return (document.querySelector('[data-testid*="send" i]') ||
            Array.from(document.querySelectorAll('button')).find(b => { var _a; return /send/i.test(b.getAttribute('aria-label') || ((_a = b.dataset) === null || _a === void 0 ? void 0 : _a.testid) || b.textContent); }));
    }
    ...
    function waitForPromptInput(callback) {
        if (!promptInputObserver) {
            const observer = new MutationObserver(() => {
                ...
                const colDiv = (promptDiv === null || promptDiv === void 0 ? void 0 : promptDiv.closest('.flex-col.items-center')) || (promptDiv === null || promptDiv === void 0 ? void 0 : promptDiv.parentElement);
                if (promptDiv && colDiv) {
    ...
    waitForPromptInput((promptDiv, colDiv) => {
        ...
        promptDiv.addEventListener('keydown', (e) => {
            if (e.key === 'Enter' && !e.shiftKey && !e.isComposing) {
                ...
                history = (0, history_1.addToHistory)(history, text);
            }
        });
        ...
        const observer = new MutationObserver(() => {
            ...
            const cd = ((pd === null || pd === void 0 ? void 0 : pd.closest('.flex-col.items-center')) || (pd === null || pd === void 0 ? void 0 : pd.parentElement));
