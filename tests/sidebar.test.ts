...
test('parses repo names from environment select', { concurrency: false }, () => {
  const html = '<select data-testid="environment-select"><option>foo/bar</option><option>foo/bar</option><option>bar/baz</option></select>';
...
test('parses repo names from repository list', { concurrency: false }, () => {
  const html = '<ul data-testid="repository-list"><li>foo/bar</li><li>foo/bar</li><li>bar/baz</li></ul>';
  const dom = new JSDOM(html, { url: 'https://example.com' });
  (globalThis as any).document = dom.window.document;
  const names = parseRepoNames();
  assert.deepStrictEqual(names, ['foo/bar', 'bar/baz']);
