...
body, html {
  ...
  function toggleDocs(hide) {
    const links = Array.from(document.querySelectorAll("a")).filter(
      (a) => a.textContent && a.textContent.includes("Docs"),
    );
  ...
  function toggleProfile(hide) {
    const node = document.querySelector(
      '[aria-label*="Profile" i], [data-testid*="profile" i], [class*="profile" i], [class*="avatar" i]',
    );
  ...
  function toggleSettingsButton(hide) {
    const link = Array.from(
      document.querySelectorAll('a[href="/codex/settings/general"]'),
    ).filter((el) => el.textContent.trim() === "Settings")[0];
  ...
  function applyOptions() {
    ...
    if (options.threeColumnMode) {
      if (!document.head.contains(columnStyle))
        document.head.appendChild(columnStyle);
    } else {
      if (columnStyle.parentNode)
        columnStyle.parentNode.removeChild(columnStyle);
    }

    const repoEl = document.getElementById("gpt-repo-sidebar");
    if (repoEl) {
      if (options.repoSidebarX !== null)
        repoEl.style.left = options.repoSidebarX + "px";
      if (options.repoSidebarY !== null)
        repoEl.style.top = options.repoSidebarY + "px";
      if (options.repoSidebarWidth !== null)
    ...
    if (verEl) {
      if (options.versionSidebarX !== null)
        verEl.style.left = options.versionSidebarX + "px";
      if (options.versionSidebarY !== null)
        verEl.style.top = options.versionSidebarY + "px";
      if (options.versionSidebarWidth !== null)
    ...
    if (statsEl) {
      if (options.statsSidebarX !== null)
        statsEl.style.left = options.statsSidebarX + "px";
      if (options.statsSidebarY !== null)
        statsEl.style.top = options.statsSidebarY + "px";
      if (options.statsSidebarWidth !== null)
  ...
  const themeSelect = createSelect("gpt-setting-theme", [
    ["light", "Light"],
    ["dark", "Dark"],
    ["oled", "OLED"],
  ]);
  ...
  const fontSelect = createSelect("gpt-setting-font", [
  ...
  modalContent.appendChild(
    createSettingGroup("Font", [fontLabel, customFontInput]),
  );
  ...
  const branchesGroup = createSettingGroup("Branches", [
    ...
    document.createElement("br"),
    createCheckbox(
      "gpt-setting-auto-archive-merged",
      "Auto-archive merged tasks",
    ),
    document.createElement("br"),
    createCheckbox(
      "gpt-setting-auto-archive-closed",
      "Auto-archive closed tasks",
    ),
  ]);
  ...
  closeWrap.appendChild(
    createButton("Close", "btn btn-secondary btn-small", "gpt-settings-close"),
  );
  ...
  histActions.appendChild(
    createButton("Clear", "btn btn-secondary btn-small", "gpt-history-clear"),
  );
  histActions.appendChild(
    createButton("Close", "btn btn-secondary btn-small", "gpt-history-close"),
  );
  ...
  statsHeader.appendChild(
    createButton("×", "btn relative btn-secondary btn-small", "gpt-stats-hide"),
  );
  ...
  const getStatsTarget = () =>
    document.querySelector(".task-row-container")?.parentElement ||
    document.body;
  ...
  previewActions.appendChild(
    createButton("Use", "btn btn-primary btn-small", "gpt-preview-use"),
  );
  previewActions.appendChild(
    createButton("Cancel", "btn btn-secondary btn-small", "gpt-preview-cancel"),
  );
  ...
  repoHeader.appendChild(
    createButton("×", "btn relative btn-secondary btn-small", "gpt-repo-hide"),
  );
  ...
  versionHeader.appendChild(
    createButton(
      "×",
      "btn relative btn-secondary btn-small",
      "gpt-version-hide",
    ),
  );
  ...
  function clearBranches(status) {
    const items = Array.from(
      versionSidebar.querySelectorAll("#gpt-version-list li"),
    );
    items.forEach((it) => {
  ...
  function bulkArchive(status) {
    ...
    function next() {
      ...
      overlay.querySelector("#gpt-progress-text").textContent =
        `${i}/${buttons.length}`;
      setTimeout(next, 750);
    }
    next();
  ...
  versionSidebar
    .querySelector("#gpt-clear-open")
    .addEventListener("click", () => {
      bulkArchive("open");
      clearBranches("Open");
    });
  versionSidebar
    .querySelector("#gpt-clear-merged")
    .addEventListener("click", () => {
      bulkArchive("merged");
      clearBranches("Merged");
    });
  versionSidebar
    .querySelector("#gpt-clear-closed")
    .addEventListener("click", () => {
      bulkArchive("closed");
      clearBranches("Closed");
    });
  versionSidebar
    .querySelector("#gpt-clear-all")
    .addEventListener("click", () => {
  ...
  function openSettings() {
    ...
    modal.querySelector("#gpt-setting-logo-text").checked =
      options.hideLogoText;
    modal.querySelector("#gpt-setting-logo-image").checked =
      options.hideLogoImage;
    modal.querySelector("#gpt-setting-profile").checked = options.hideProfile;
    modal.querySelector("#gpt-setting-settings-button").checked =
      options.hideSettings;
    modal.querySelector("#gpt-setting-three-column").checked =
      options.threeColumnMode;
    modal.querySelector("#gpt-setting-auto-updates").checked =
      options.autoCheckUpdates;
    modal.querySelector("#gpt-setting-disable-history").checked =
      options.disableHistory;
    modal.querySelector("#gpt-setting-history-limit").value = String(
      options.historyLimit,
    );
    modal.querySelector("#gpt-setting-show-repos").checked =
      options.showRepoSidebar;
    modal.querySelector("#gpt-setting-show-versions").checked =
      options.showVersionSidebar;
    modal.querySelector("#gpt-setting-show-stats").checked =
      options.showStatsSidebar;
    modal.querySelector("#gpt-setting-clear-closed").checked =
      options.clearClosedBranches;
    modal.querySelector("#gpt-setting-clear-merged").checked =
      options.clearMergedBranches;
    modal.querySelector("#gpt-setting-clear-open").checked =
      options.clearOpenBranches;
    modal.querySelector("#gpt-setting-auto-archive-merged").checked =
      options.autoArchiveMerged;
    modal.querySelector("#gpt-setting-auto-archive-closed").checked =
      options.autoArchiveClosed;
  ...
  modal
    .querySelector("#gpt-settings-close")
    .addEventListener("click", () => modal.classList.remove("show"));
  modal.addEventListener("click", (e) => {
    if (e.target === modal) modal.classList.remove("show");
  });
  historyModal
    .querySelector("#gpt-history-close")
    .addEventListener("click", () => historyModal.classList.remove("show"));
  historyModal.addEventListener("click", (e) => {
    if (e.target === historyModal) historyModal.classList.remove("show");
  });
  historyModal
    .querySelector("#gpt-history-clear")
    .addEventListener("click", () => {
      history = [];
      saveHistory(history);
      renderHistory();
  ...
  modal
    .querySelector("#gpt-setting-custom-font")
  ...
  modal
    .querySelector("#gpt-setting-logo-text")
  ...
  modal
    .querySelector("#gpt-setting-logo-image")
    .addEventListener("change", (e) => {
  ...
  modal
    .querySelector("#gpt-setting-profile")
    .addEventListener("change", (e) => {
  ...
  modal
    .querySelector("#gpt-setting-settings-button")
    .addEventListener("change", (e) => {
  ...
  modal
    .querySelector("#gpt-setting-three-column")
    .addEventListener("change", (e) => {
  ...
  modal
    .querySelector("#gpt-setting-auto-updates")
    .addEventListener("change", (e) => {
      options.autoCheckUpdates = e.target.checked;
      saveOptions(options);
    });
  modal
    .querySelector("#gpt-setting-disable-history")
    .addEventListener("change", (e) => {
      options.disableHistory = e.target.checked;
      saveOptions(options);
    });
  modal
    .querySelector("#gpt-setting-history-limit")
    .addEventListener("change", (e) => {
      options.historyLimit = parseInt(e.target.value, 10) || 1;
      saveOptions(options);
    });
  modal
    .querySelector("#gpt-setting-show-repos")
    .addEventListener("change", (e) => {
  ...
  modal
    .querySelector("#gpt-setting-show-versions")
    .addEventListener("change", (e) => {
  ...
  modal
    .querySelector("#gpt-setting-show-stats")
    .addEventListener("change", (e) => {
  ...
  modal
    .querySelector("#gpt-setting-clear-closed")
    .addEventListener("change", (e) => {
      options.clearClosedBranches = e.target.checked;
      saveOptions(options);
    });
  modal
    .querySelector("#gpt-setting-clear-merged")
    .addEventListener("change", (e) => {
      options.clearMergedBranches = e.target.checked;
      saveOptions(options);
    });
  modal
    .querySelector("#gpt-setting-clear-open")
    .addEventListener("change", (e) => {
      options.clearOpenBranches = e.target.checked;
      saveOptions(options);
    });
  modal
    .querySelector("#gpt-setting-auto-archive-merged")
    .addEventListener("change", (e) => {
      options.autoArchiveMerged = e.target.checked;
      saveOptions(options);
    });
  modal
    .querySelector("#gpt-setting-auto-archive-closed")
    .addEventListener("change", (e) => {
  ...
  modal.querySelector("#gpt-reset-windows").addEventListener("click", () => {
    options.repoSidebarX =
      options.repoSidebarY =
      options.repoSidebarWidth =
      options.repoSidebarHeight =
        null;
    options.versionSidebarX =
      options.versionSidebarY =
      options.versionSidebarWidth =
      options.versionSidebarHeight =
        null;
    options.statsSidebarX =
      options.statsSidebarY =
      options.statsSidebarWidth =
      options.statsSidebarHeight =
        null;
    saveOptions(options);
    applyOptions();
  });
  ...
  if (
    options.autoCheckUpdates &&
    (!options.lastUpdateCheck || now - options.lastUpdateCheck > 86400000)
  ) {
  ...
  function findSendButton() {
    return (
      ...
      Array.from(document.querySelectorAll("button")).find((b) =>
        /send/i.test(
          b.getAttribute("aria-label") || b.dataset?.testid || b.textContent,
        ),
      )
    );
  }
  ...
  function injectDropdown(promptDiv, colDiv) {
    ...
    dropdown.className =
      "flex h-8 w-full rounded-md border bg-background px-3 py-1 text-sm shadow-sm focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring";
  ...
  function waitForPromptInput(callback) {
    ...
    const observer = new MutationObserver(() => {
      ...
      const colDiv =
        promptDiv?.closest(".flex-col.items-center") ||
        promptDiv?.parentElement;
      if (promptDiv && colDiv) {
        observer.disconnect();
    ...
    const promptDiv = findPromptInput();
    const colDiv =
      promptDiv?.closest(".flex-col.items-center") || promptDiv?.parentElement;
    if (promptDiv && colDiv) {
      observer.disconnect();
      const i = observers.indexOf(observer);
      if (i !== -1) observers.splice(i, 1);
  ...
  waitForPromptInput((promptDiv, colDiv) => {
    ...
    promptDiv.addEventListener("keydown", async (e) => {
      if (e.key === "Enter" && !e.shiftKey && !e.isComposing) {
        const text =
          promptDiv instanceof HTMLTextAreaElement
            ? promptDiv.value
            : promptDiv.textContent;
        history = await addToHistory(history, text);
      }
    });
